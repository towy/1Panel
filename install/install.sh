...
function Install_Compose(){
    ...
    if [[ $? -ne 0 ]]; then
        ...
        if [ -z "$COMPOSE_VERSION" ]; then
            log "无法获取最新的 docker-compose 版本信息"
            exit 1
        fi
        
        # 下载对应架构的Docker Compose
        curl -L "https://github.com/docker/compose/releases/download/${COMPOSE_VERSION}/docker-compose-${arch}" -o /usr/local/bin/docker-compose 2>&1 | tee -a ${CURRENT_DIR}/install.log
