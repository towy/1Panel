...
main() {
  ...
  # --- 检查核心依赖 ---
  if ! command -v git &> /dev/null; then die "git 未安装，请先安装 git。"; fi

  # --- 获取 1Panel 安装路径 ---
  local install_path="/opt/1panel"
  ...
  if [[ ! -d "$install_path" ]]; then die "1Panel 安装路径不存在: $install_path"; fi
  local apps_local_dir="${install_path}/resource/apps/local"
  mkdir -p "$apps_local_dir"
  ...
  sync_to_destination "$staging_dir" "$apps_local_dir" "$FORCE_UPDATE_MODE"
  echo "阶段 2 完成。"
